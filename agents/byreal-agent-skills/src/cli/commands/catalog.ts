...
const CAPABILITIES: Capability[] = [
  ...
  },
  {
    ...
    name: 'Open Position',
    description: 'Open a new CLMM position. Two modes: --amount (token amount) or --amount-usd (USD budget, auto-splits into tokenA/B). --dry-run checks wallet balance and reports deficit with swap suggestions. Add --auto-swap to deposit a single token (--base + --amount); the backend swaps the optimal portion into the other side.',
    category: 'execute',
    auth_required: true,
    ...
    params: [
      ...
      { name: 'base', type: 'string', required: false, description: 'Base token (required with --amount or --auto-swap)', enum: ['MintA', 'MintB'] },
      { name: 'amount', type: 'string', required: false, description: 'Amount of base token (UI format). Mutually exclusive with --amount-usd.' },
      { name: 'amount-usd', type: 'string', required: false, description: 'Investment in USD. Auto-calculates token A/B split. Mutually exclusive with --amount. Not supported with --auto-swap.' },
      { name: 'slippage', type: 'integer', required: false, description: 'Slippage tolerance in basis points' },
      { name: 'raw', type: 'boolean', required: false, description: 'Amount is already in raw (smallest unit) format' },
      { name: 'auto-swap', type: 'boolean', required: false, description: 'Auto Swap: deposit only --base; backend swaps the optimal portion into the other side' },
  ...
  },
  {
    ...
    description: 'Add liquidity to an existing position. Two modes: --amount (token amount with --base) or --amount-usd (USD budget, auto-splits). --dry-run checks wallet balance. Add --auto-swap to deposit a single token (--base + --amount); the backend swaps the optimal portion into the other side.',
    category: 'execute',
    auth_required: true,
  ...
  },
  {
    ...
    description: 'Partially remove liquidity from a position (keeps position NFT open, unlike close which burns it). Two modes: --percentage (by ratio) or --amount-usd (by USD value). Add --auto-swap with --output-mint to receive the result as a single token.',
    category: 'execute',
    auth_required: true,
    ...
    params: [
      ...
      { name: 'amount-usd', type: 'string', required: false, description: 'USD amount of liquidity to remove. Auto-calculates percentage. Mutually exclusive with --percentage. Not supported with --auto-swap.' },
      { name: 'slippage', type: 'integer', required: false, description: 'Slippage tolerance in basis points' },
      { name: 'auto-swap', type: 'boolean', required: false, description: 'Auto Swap: collapse withdrawn token A + B into a single output token' },
      { name: 'output-mint', type: 'string', required: false, description: 'With --auto-swap: target token mint to receive (must be one of the pool tokens)' },
      { name: 'dry-run', type: 'boolean', required: false, description: 'Preview without executing' },
  ...
  },
  {
    ...
    name: 'Close Position',
    description: 'Close a position (remove all liquidity and burn position NFT). Add --auto-swap with --output-mint to receive the result as a single token.',
    category: 'execute',
    auth_required: true,
